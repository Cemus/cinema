...
async function loadData() {
  const url =
    "https://docs.google.com/spreadsheets/d/1OeoL8SLc6uDoicWNEbwT4qwVMq9gEz0euL19bSqcp7g/edit#gid=0";
...
function usineObjet(quiPropose) {
  ...
  duree = image.nextElementSibling;
...
function listeObjet() {
  ...
  array = [];
  while (
    document.getElementById("0R" + i).nextElementSibling.textContent ==
      "Helo" ||
    document.getElementById("0R" + i).nextElementSibling.textContent ==
      "Kevouche"
  ) {
...
function createCard(array) {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    if (listeFilm[navigation].image !== "") {
      ...
      card[
        i
      ].style.backgroundImage = `url(https://www.ulyces.co/wp-content/uploads/2018/01/posters_2_1500-400x600.jpg)`;
    }
    titre[i].innerHTML = listeFilm[navigation].titre;
    duree[i].innerHTML = `(${listeFilm[navigation].duree})`;
    listeFilm[navigation].synopsis !== ""
      ? (synopsis[i].innerHTML = listeFilm[navigation].synopsis)
      : (synopsis[i].innerHTML = "Il n'y a pas de description ;)");
    listeFilm[navigation].notePresse !== ""
      ? (notePresse[i].innerHTML = listeFilm[navigation].notePresse)
      : (notePresse[i].innerHTML = "abs");
    listeFilm[navigation].noteSpectateur !== ""
      ? (noteSpectateur[i].innerHTML = listeFilm[navigation].noteSpectateur)
      : (noteSpectateur[i].innerHTML = "abs");
  }
}
...
function createRandomCard(array) {
  ...
  duree[3].innerHTML = `(${listeFilm[0].duree})`;
  listeFilm[0].synopsis !== ""
    ? (synopsis[3].innerHTML = listeFilm[0].synopsis)
    : (synopsis[3].innerHTML = "Il n'y a pas de description ;)");
  listeFilm[0].notePresse !== ""
    ? (notePresse[3].innerHTML = listeFilm[0].notePresse)
    : (notePresse[3].innerHTML = "abs");
  listeFilm[0].noteSpectateur !== ""
    ? (noteSpectateur[3].innerHTML = listeFilm[0].noteSpectateur)
    : (noteSpectateur[3].innerHTML = "abs");
}
